...
<head>
    ...
    <title>Data Warga - Panel Admin</title>
...
<body class="bg-slate-50 text-slate-800 antialiased min-h-screen flex flex-col">
    ...
    <nav class="bg-white/90 backdrop-blur-md border-b border-slate-200 sticky top-0 z-50">
        <div class="container mx-auto px-4 h-16 flex justify-between items-center">
            <div class="flex items-center gap-3">
                ...
                <span class="bg-blue-100 text-blue-700 text-xs font-bold px-2 py-1 rounded-md border border-blue-200">ADMIN</span>
            </div>

            <div class="hidden md:flex items-center gap-1 bg-slate-100 p-1 rounded-lg">
                <a href="{{ route('admin.dashboard') }}" class="px-4 py-1.5 rounded-md text-sm font-medium text-slate-600 hover:text-slate-900 transition">
                    Laporan Masuk
                </a>
                <a href="{{ route('admin.warga.index') }}" class="px-4 py-1.5 rounded-md text-sm font-medium bg-white text-blue-700 shadow-sm transition">
                    Data Warga
                </a>
            </div>
            ...
            <div class="flex items-center gap-4">
                <div class="text-right hidden md:block">
                    ...
                    <a href="{{ route('profile.edit') }}" class="text-sm font-bold text-slate-800 hover:text-blue-600 transition">
                        {{ Auth::user()->name }}
                    </a>
                </div>
    ...
    <main class="container mx-auto px-4 py-8 flex-grow">
        ...
        <div class="flex justify-between items-end mb-6">
            <div>
                <h1 class="text-2xl font-bold text-slate-800">Manajemen Warga</h1>
                <p class="text-slate-500 text-sm mt-1">Daftar penduduk yang telah mendaftar di sistem.</p>
            </div>
            <div class="text-sm font-bold text-slate-600 bg-white px-4 py-2 rounded-lg border border-slate-200 shadow-sm">
                        ...
                        @forelse($wargas as $warga)
                            <tr class="hover:bg-slate-50 transition">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <form action="{{ route('admin.warga.destroy', $warga->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus data warga ini? Semua laporan terkait juga akan terhapus.');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg text-xs font-bold transition">
                                            Hapus Akun
                                        </button>
                                    </form>
                                </td>
